...
export default function OverviewPage() {
  ...
  return <div className="overview-page">
    <div className="page-heading">
      <div><div className="eyebrow">YOUR COMMUNITY, AT A GLANCE</div><h1>Overview<span className="heading-dot">.</span></h1><p>The big picture. Every detail that matters.</p></div>
    ...
    {!data ? (!error && <div className="loading-grid" role="status" aria-label="Loading overview">{[0,1,2,3].map(i => <div className="skeleton" key={i} />)}<div className="skeleton skeleton-chart" /><span className="sr-only">Loading overview…</span></div>) : <>
      <div className="overview-status"><span className={`status-pill ${status === 'Live updates' ? 'live' : ''}`}><span className="status-dot" />{status}</span><span>Last received {updated ? timeLabel(updated) : '—'}</span><span className="session-label">CURRENT SESSION <span>↗</span></span></div>
      <div className="metric-grid">
        <StatCard label="Total guilds" value={data.guild_count ?? data.servers ?? '—'} icon="guilds" sub="Connected communities" />
        <StatCard label="Total users" value={data.users ?? '—'} icon="users" sub="Across your network" />
      ...
      <div className="analytics-grid">
        <section className="dashboard-panel telemetry-panel">
          <div className="panel-heading"><div><div className="section-kicker">PERFORMANCE</div><h2>Network telemetry</h2></div><span className="small-badge">{samples.length} sample{samples.length === 1 ? "" : "s"}</span></div>
          ...
          <div className="chart-footnote">{samples.length < 2 ? 'Collecting measurements. The trend will appear as updates arrive.' : 'Observed in this tab · up to 120 recent samples · no historical data available'}</div>
        ...
        <section className="dashboard-panel resources-panel"><div className="panel-heading"><div><div className="section-kicker">INFRASTRUCTURE</div><h2>System resources</h2></div><Icon name="control" /></div>
          ...
          <div className="resource-note"><Icon name="activity" size={16} /><span>Resource values reflect the latest received snapshot.</span></div>
        </section>
      </div>
      <div className="bottom-grid"><section className="dashboard-panel"><div className="panel-heading"><div><div className="section-kicker">BOT MODULES</div><h2>Loaded cogs <span className="count-badge">{data.cog_count ?? cogs.length}</span></h2></div><label className="cog-search"><Icon name="search" size={15} /><input aria-label="Filter loaded cogs" placeholder="Filter modules…" value={cogQuery} onChange={e => setCogQuery(e.target.value)} /></label></div><div className="cog-list">{cogs.filter(c => String(c).toLowerCase().includes(cogQuery.toLowerCase())).map(c => <span className="cog-chip" key={c}><i />{c}</span>)}{!cogs.length && <p className="text-sm text-white/40">No loaded modules reported.</p>}{cogs.length > 0 && !cogs.some(c => String(c).toLowerCase().includes(cogQuery.toLowerCase())) && <p className="text-sm text-white/40">No modules match your search.</p>}</div></section>
      <section className="dashboard-panel shortcuts-panel"><div className="section-kicker">QUICK ACCESS</div><h2>Your next move</h2>{[{ to: '/moderation', permission: 'moderation.view', icon: 'shield', label: 'Moderation', sub: 'Keep your community safe' }, { to: '/economy', permission: 'economy.view', icon: 'economy', label: 'Economy', sub: 'Explore balances & accounts' }, { to: '/bot-control', permission: 'overview.view', icon: 'control', label: 'Bot control', sub: 'Manage your infrastructure' }].filter(item => hasPermission(item.permission)).map(item => <Link key={item.to} to={item.to} className="shortcut"><span className="shortcut-icon"><Icon name={item.icon} /></span><span><strong>{item.label}</strong><small>{item.sub}</small></span><Icon name="arrow" size={16} /></Link>)}</section></div>
    </>}
  </div>;
}
